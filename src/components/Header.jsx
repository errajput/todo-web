...
export default function Header() {
  ...
  return (
    <header className="bg-gray-400 text-white shadow-md  p-2 flex justify-between items-center  sticky top-0 z-50">
      <div className="flex items-center space-x-3">
        <img src="/todoIcon.webp" alt="Todo APP" className="w-12 h-12" />{" "}
        <Link href="/">
          <h1
      ...
      <nav className="hidden md:flex space-x-9">
        <>
          <Link
            href="/register"
            className="cursor-pointer relative transition duration-300 hover:text-white  font-bold
      ...
      {/* Mobile Menu */}
      {menuOpen && (
        <div className="absolute top-16 right-0 w-50  bg-gray-300 text-black flex flex-col items-center space-y-4 py-4 shadow-md md:hidden">
          {!user?.isLogin ? (
            <>
              <Link
                href="/register"
